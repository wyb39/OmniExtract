"""Compact, layout-aware PDF to Markdown parser.
...
def _opendoc_model_base(model_dir: str | Path | None) -> Path:
    if model_dir is not None:
        ...
        configured_dir = os.environ.get("OMNIEXTRACT_OPENDOC_MODEL_DIR")
        if configured_dir:
            base_dir = Path(configured_dir)
        else:
            project_root = Path(__file__).resolve().parents[1]
            base_dir = project_root / "models" / "opendoc"
            # Transitional compatibility for the model bundle that predates
            # migration. The root models directory takes precedence once it
            # has been provisioned.
            legacy_dir = project_root / "pdf_parsing_experiment" / "models" / "opendoc"
...
def _ensure_opendoc_models(
    model_dir: Path,
    required_files: tuple[str, ...],
    auto_download: bool,
) -> None:
    """Raise an actionable error before OpenDoc starts when downloads are off."""
    ...
    if missing:
        raise FileNotFoundError(
            "OpenDoc model files are missing from "
            f"{model_dir}: {', '.join(missing)}. "
            "Provision the models, set OMNIEXTRACT_OPENDOC_MODEL_DIR, "
            "or enable auto_download explicitly."
        )
...
def convert_pdf(
    ...
    *,
    ...
    try:
        from .pdf_markdown_renderer import render_document
    except ImportError:
        from pdf_markdown_renderer import render_document
