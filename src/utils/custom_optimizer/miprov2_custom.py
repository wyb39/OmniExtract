...
class MIPROv2_Custom(Teleprompter):
    def __init__(
        self,
        metric: Callable,
    ...
    def compile(
        ...
        *,
        ...
        demo_candidates = self._modify_demo_candidates(demo = demo_candidates, trainset=trainset)
    ...
    def _modify_demo_candidates(self,demo, trainset):
        logger.info(trainset[0]._input_keys)
        output = {}
        list_inputs = list(trainset[0]._input_keys)
        for program_key in demo:
            current_program_list = demo[program_key]
            output_list = []
            for current in current_program_list:
                ...
                if len(current) == 0:
                    ...
                    continue
                for item in current:
                    for exam in trainset:
                        flag = True
                        for input_key in list_inputs:
                            if exam[input_key] != item[input_key]:
                                flag = False
                                break
                        if flag:
                            output_current.append(exam)
                            break
                output_list.append(output_current)
            output[program_key] = output_list
        return output
