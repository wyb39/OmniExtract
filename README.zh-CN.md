...
> **PDF 输入使用集成的 Hybrid/OpenDoc 解析器。**
> Hybrid 是生产环境的默认选项。要将生产流程切换为完整的 OpenDoc，
> 请将 `src/parsing/articleUtil.py` 中的 `PDF_PARSER_BACKEND` 修改为 `"opendoc"`。
> 此开关有意保持为内部选项，不会改变现有的 CLI/API/YAML 输入参数。
> 请在 `models/opendoc` 下准备模型文件，或在运行前设置
