"""
gui — Jinja/Flask web UI for OmniExtract.
...
Design notes:
  * Pure Flask + Jinja2 templates (no Dash).
  ...
  * Recent-task cards are rendered server-side from gui/runs/<module>.
...
# Put the project root AND the gui dir on sys.path so that:
#   * gui / src are importable as packages
#   * gui's internal flat imports (e.g. `from process_manager import ...`) resolve
# Works whether launched via `python gui/app.py` or `python -m gui.app`.
...
from gui import task_service, taskcard
...
def main():
    ...
    use_prod = os.environ.get("GUI_PROD", "1") == "1"
    if use_prod:
        try:
            from waitress import serve
            print(f"gui (waitress) serving on http://{host}:{port}")
